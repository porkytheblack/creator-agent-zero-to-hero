...
export const chapters: CourseChapter[] = ([
    ...
    units: [
      {
        ...
        mentalModel: { plain: "A rehearsal uses marked prop documents that follow the same receiving and filing process as the real shoot.", technical: "Provider-shaped fixtures exercise HTTP decoding, normalization, deduplication, scoring, and repository persistence deterministically. They are labeled synthetic in provenance.", connection: "createDemoApifyFetch replaces the injected HTTP function while the same ApifyResearchClient, normalizer, and repositories handle both fixture and live rows." },
        sections: [
    ...
    units: [
      ...
      },
      {
        ...
        sections: [
          ...
          { title: "Use deterministic model/provider seams", paragraphs: ["For workflow tests, replace the model or assert tool-level orchestration with deterministic responses. Replace Apify HTTP and image-provider calls through their explicit injected seams while still passing through validation and persistence.", "Do not let CI spend provider credits or depend on internet volatility. Live smoke commands are opt-in and hard-bounded."] },
      ...
      {
        id: "capstone-proof",
        title: "Capstone: prove the complete boundary",
        duration: "180 min",
        promise: "You will produce evidence that you can run, inspect, explain, and safely change the system—not merely finish its pages.",
        mentalModel: { plain: "The final review is a production rehearsal with marked props, a recorded call sheet, and a clear sign-off standard.", technical: "A capstone combines deterministic fixtures, automated checks, durable-state inspection, exact-version approval, and an oral architecture trace into one reproducible proof.", connection: "This is the learner-facing version of the teaching guide rubric. It uses Sharlet’s checked-in fixtures and tests before any optional live provider smoke." },
        sections: [
          { title: "Build one evidence packet", paragraphs: ["Use a fictional brand and the deterministic Apify fixture sweep. Add one disposable reference asset and produce one review-ready packet whose evidence and source-asset lineage can be inspected.", "Do not use a live provider until the fixture path succeeds. The capstone is about system ownership, not spending money."] },
          { title: "Prove recovery and approval", paragraphs: ["Run the same logical request twice and prove the durable identity prevents two competing outputs. Approve one exact version, create a new version, and prove the previous approval cannot authorize it.", "Capture the record IDs, statuses, and the smallest tests that demonstrate each invariant. Screenshots alone are not enough."] },
          { title: "Explain the whole run", paragraphs: ["Walk from creator intent to Foundry, Glove, the Effect integration, repository persistence, creator review, and the approval boundary. At every transition name the input, output, possible failure, and durable record.", "Finish by stating what the model decides, what software guarantees, what survives restart, and what the agent is never authorized to do."] }
        ],
        files: [{ path: "docs/architecture/run-trace.md", reason: "Use the trace as the oral-exam spine." }, { path: "packages/integrations/src/apify.test.ts", reason: "The deterministic acquisition proof." }, { path: "packages/database/src/schema.ts", reason: "Durable identities and exact-version records." }, { path: "apps/runtime/agents/sharlet/agent.ts", reason: "The complete Glove and Foundry capability surface." }],
        steps: [
          { action: "Create the clean baseline", detail: "Install dependencies and prove the complete repository is healthy before changing it.", command: "pnpm install\npnpm check", expected: "Lint, typecheck, tests, and production builds all pass." },
          { action: "Run the fixture research path", detail: "Start PostgreSQL, migrate it, and seed provider-shaped fixture observations.", command: "docker compose up -d postgres\npnpm --filter @sharlet/database db:migrate\nSHARLET_WORKSPACE_ID=course-capstone pnpm --filter @sharlet/database demo:seed", expected: "The seed reports the workspace, four sources, persisted observations, and fixture: true." },
          { action: "Prove the high-risk invariants", detail: "Run the domain, integration, database-adapter, and runtime tests. Save the passing command output in LEARNING-NOTES.md.", command: "pnpm --filter @sharlet/domain test\npnpm --filter @sharlet/integrations test\npnpm --filter @sharlet/database test\npnpm --filter @sharlet/runtime test", expected: "All deterministic suites pass without provider credentials." },
          { action: "Present the capstone", detail: "Demonstrate the packet, evidence lineage, duplicate-run behavior, approval invalidation, and the complete architecture trace. Link every claim to a record, test, or exact file." }
        ],
        trace: ["Fixture evidence", "One claimed run", "Glove tool boundary", "Persisted packet", "Exact-version approval", "Edit invalidates approval", "Creator explains owners"],
        checks: ["The clean repository passes pnpm check", "Evidence and reference lineage are inspectable", "Duplicate activation does not create competing work", "An edit invalidates old approval", "I can explain every boundary without saying ‘the AI does it’"],
        quiz: { question: "What is the strongest proof that the learner understands the system?", options: ["Every course checkbox is selected", "They can reproduce a fixture run, show durable invariants, change one boundary, and explain the trace", "They have a provider API key", "They copied the production repository"], answer: 1, explanation: "Learning transfer requires a reproducible outcome, evidence from the system, and an explanation of ownership—not page completion." }
      }
    ]
  },
  {
    ...
    units: [
      ...
      },
      {
        ...
        mentalModel: { plain: "The producer asks the archive for ‘recent approved cuts,’ not for shelf 7, box 14, row 3.", technical: "Sharlet repository functions expose domain operations, hide Drizzle queries, and return typed Effect programs. Database client selection happens in the database client module.", connection: "Agent tools import repository functions. Those functions use the configured PGlite or PostgreSQL client without exposing SQL to Glove or Foundry." },
        sections: [
          ...
          { title: "Swap storage at the client boundary", paragraphs: ["Local single-process exploration may use PGlite. Autonomous subprocesses and production use PostgreSQL. Repository call sites stay unchanged because client selection is centralized.", "PGlite is not a shared multi-process production database. If scheduled agents run in subprocesses, point them at PostgreSQL so every process sees the same state."] }
        ],
    ...
    units: [
      {
        ...
        sections: [
          ...
          { title: "Install before you verify", paragraphs: ["On macOS, install Git from Xcode Command Line Tools, Node from nodejs.org, pnpm with Corepack, and Docker Desktop only when you reach PostgreSQL. On Windows, use Git for Windows, the Node LTS installer, Corepack, and Docker Desktop with WSL 2. On Linux, use your distribution package manager for Git and Docker, then the Node LTS installer instructions.", "Close and reopen the terminal after an installer changes your PATH. If a version command still fails, fix that one installation before cloning the repository."] },
        ...
        steps: [
          { action: "Install the workbench", detail: "Install Node 20 or newer and Git first. Enable pnpm with Corepack. Docker is required when the course reaches PostgreSQL.", command: "corepack enable\nnode --version\npnpm --version\ngit --version", expected: "Three version strings after Corepack succeeds. Node must be 20 or newer." },
          ...
          { action: "Create a real checkpoint", detail: "Create LEARNING-NOTES.md in your editor, write the course URL on its first line, then commit that one deliberate change.", command: "git add LEARNING-NOTES.md\ngit commit -m \"Add learning notes\"\ngit status", expected: "Git creates one commit and reports a clean working tree. Do not run git init inside the cloned repository." }
        ],
  ...
  },
  {
    ...
    milestone: "One fixture research workflow composes across packages with typed Effect failures and replaceable provider seams.",
    units: [
      {
        id: "monorepo-graph",
      ...
      },
      {
        ...
        mentalModel: { plain: "The production plan states the result, named things that can go wrong, and the crew or equipment required before work begins.", technical: "Effect.Effect<A, E, R> describes a lazy program that can succeed with A, fail with E, and require R. Sharlet currently uses Effect for lazy composition and typed failures while injecting several clients explicitly through constructors and function arguments.", connection: "The Apify client returns an Effect with a typed provider failure. The Glove research tool composes that Effect with repository Effects, then the runtime executes the completed program." },
        sections: [
          { title: "Effects are descriptions until run", paragraphs: ["Effect.gen composes operations without immediately executing them. In Sharlet, the Glove tool creates its Apify client explicitly, composes client and repository Effects, and calls Effect.runPromise at the application boundary.", "Effect also supports Context services and Layers. Those are useful when an application has many interchangeable implementations, but they are not present in the referenced Sharlet research path today. Learn the code that exists before proposing that refactor."] },
          ...
          { title: "Name the dependency style you actually see", paragraphs: ["ApifyResearchClient is a constructor-injected adapter. createDemoApifyFetch replaces its HTTP dependency for deterministic tests, while the production tool uses global fetch and a real token. Repository functions return Effects directly.", "A future Context.Tag and Layer refactor could make requirements visible in the R channel. Treat that as an architectural exercise—not as a description of code that already exists."] }
        ],
        ...
        steps: [
          { action: "Read one real type aloud", detail: "Read ApifyResearchClient.harvest: it succeeds with normalized observations, fails with ResearchProviderError, and currently has no Effect environment requirement." },
          { action: "Trace explicit injection", detail: "Follow the fetch dependency from the constructor into createDemoApifyFetch and the production research tool." },
          { action: "Run the typed failure test", detail: "Execute the integration tests and locate the missing-token assertion.", command: "pnpm --filter @sharlet/integrations test", expected: "The Apify tests pass without a provider credential." },
          { action: "Design—not pretend—a Layer refactor", detail: "On paper, define the Research service contract, fixture Layer, and live Layer you would add. List the files that would change before writing code." }
        ],
        checks: ["I can read A, E, and R", "I can distinguish constructor injection from Effect Layers", "I found the real Effect.runPromise boundary", "Retry policy distinguishes failure classes"],
        quiz: { question: "How does Sharlet currently replace live Apify HTTP behavior in deterministic tests?", options: ["It switches an ApifyResearch Layer", "It injects createDemoApifyFetch into ApifyResearchClient", "It asks the model not to use the network", "It changes PostgreSQL schemas"], answer: 1, explanation: "The current code uses explicit constructor injection. Effect Layers would be a possible future refactor, not the implementation being referenced." }
      },
      {
        id: "effect-workflow",
        ...
        steps: [
          ...
          { action: "Run with injected fixtures", detail: "Confirm deterministic output and no live network call." },
